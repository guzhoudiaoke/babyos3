...
int64 (*syscall_t::s_system_call_table[])(trap_frame_t* frame) = {
    syscall_t::sys_print,
    //syscall_t::sys_fork,
    //syscall_t::sys_exec,
    //syscall_t::sys_mmap,
...
int64 syscall_t::sys_print(trap_frame_t* frame)
{
    color_ref_t color = (color_ref_t) get_argument(frame, 0);

    char buffer[512] = {0};
    //char* va = (char *) PA2VA(os()->get_mm()->va_2_pa((void *) frame->ecx));
    char* va = (char *) get_argument(frame, 1);

    os()->uart()->kprintf("sys print: %lx, %p\n", color, va);
