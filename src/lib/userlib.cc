...
{
    int ret = 0;
    __asm__ volatile("int $0x80" : "=a" (ret) : "a" (syscall_t::PRINT), "D" (color), "S" (str) );
    return ret;
}
...
{
    int ret = 0;
    __asm__ volatile("int $0x80" : "=a" (ret) : "a" (syscall_t::EXEC), "D" (path), "S" (arg));
    return ret;
}

void *userlib_t::mmap(uint64 addr, uint64 len, uint32 prot, uint32 flags)
{
    uint64 ret = 0;
    __asm__ volatile(
        "int $0x80"
        : "=a" (ret)
        : "a" (syscall_t::MMAP), "D" (addr), "S" (len), "d" (prot), "c" (flags));
...
{
    __asm__ volatile("int $0x80" : : "D" (val), "a" (syscall_t::EXIT));
}

void userlib_t::wait(uint32 pid)
{
    __asm__ volatile("int $0x80" : : "D" (pid), "a" (syscall_t::WAIT));
}
...
{
    __asm__ volatile("int $0x80" : : "D" (second), "a" (syscall_t::SLEEP));
}

//void userlib_t::kill(uint32 pid, uint32 sig)
//{
//    __asm__ volatile("int $0x80" : : "b" (pid), "c"(sig), "a" (syscall_t::KILL));
//}

//void userlib_t::signal(uint32 sig, sighandler_t handler)
//{
//    __asm__ volatile("int $0x80" : : "b" (sig), "c"(handler), "a" (syscall_t::SIGNAL));
//}
...
{
    uint32 ret = 0;
    __asm__ volatile("int $0x80" : "=a" (ret) : "a" (syscall_t::OPEN), "D" (path), "S" (mode));
    return ret;
}
...
{
    uint32 ret = 0;
    __asm__ volatile("int $0x80" : "=a" (ret) : "a" (syscall_t::CLOSE), "D" (fd));
    return ret;
}

int64 userlib_t::read(int fd, void* buf, uint64 size)
{
    uint32 ret = 0;
    __asm__ volatile(
        "int $0x80"
        : "=a" (ret)
        : "a" (syscall_t::READ), "D" (fd), "S" ((uint64) buf), "d" (size));
    return ret;
}

int64 userlib_t::write(int fd, void* buf, uint64 size)
{
    uint32 ret = 0;
    __asm__ volatile(
        "int $0x80"
        : "=a" (ret)
        : "a" (syscall_t::WRITE), "D" (fd), "S" ((uint64) buf), "d" (size));
    return ret;
}
...
{
    uint32 ret = 0;
    __asm__ volatile("int $0x80" : "=a" (ret) : "a" (syscall_t::MKDIR), "D" (path));
    return ret;
}
...
int userlib_t::link(const char* path_old, const char* path_new)
{
    ...
    __asm__ volatile(
        "int $0x80"
        : "=a" (ret)
        : "a" (syscall_t::LINK), "D" ((uint64) path_old), "S" ((uint64) path_new));
    return ret;
}
...
{
    uint32 ret = 0;
    __asm__ volatile("int $0x80" : "=a" (ret) : "a" (syscall_t::UNLINK), "D" (path));
    return ret;
}
...
{
    uint32 ret = 0;
    __asm__ volatile("int $0x80" : "=a" (ret) : "a" (syscall_t::MKNOD), "D" (path), "S" (major), "d" (minor));
    return ret;
}
...
{
    uint32 ret = 0;
    __asm__ volatile("int $0x80" : "=a" (ret) : "a" (syscall_t::DUP), "D" (fd));
    return ret;
}
...
{
    uint32 ret = 0;
    __asm__ volatile("int $0x80" : "=a" (ret) : "a" (syscall_t::CHDIR), "D" (path));
    return ret;
}
...
{
    uint32 ret = 0;
    __asm__ volatile("int $0x80" : "=a" (ret) : "a" (syscall_t::STAT), "D" (fd), "S" (st));
    return ret;
}
